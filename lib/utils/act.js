  ...
  init: (configs) => {
    ...
    const handlers = {
      QueryableActivity: (activity, instructions) => {
        ...
        */
        let activityParts = instructions.Resource.split(":");
        
        let payload = {
          input: DOT.get(activity.input, instructions.InputPath, null),
          queryable: DOT.get(activity.input, instructions.Queryable, null),
          meta: DOT.get(activity.input, "meta", null),
      ...
      SubStep: (activity, instructions) => {
        ...
        */
        ...
        let payload = {
          initial: DOT.get(activity.input, instructions.InputPath, null),
          meta: {
            /*
            ...
            */
            ...
            parent: {
              ...
              */
              name: DOT.get(activity.input, "$.meta.name", null),
              arn: DOT.get(activity.input, "$.meta.arn", null),

              stage: DOT.get(activity.input, "$.meta.stage", null),
              alias: DOT.get(activity.input, "$.meta.alias", null),
              version: DOT.get(activity.input, "$.meta.version", null),
              region: DOT.get(activity.input, "$.meta.region", null),
            }
          }
        }
    ...
    function handleActivity(arn, activity) {
      ...
      */
      return handler(activity, instructions).catch((err) => {
        /*
